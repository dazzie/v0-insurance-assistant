...
export function FormattedResponse({ content }: FormattedResponseProps) {
  ...
  const parseInlineFormatting = (text: string): React.ReactNode[] => {
    ...
    const segments = currentText.split(/(__(?:BOLD|STRONG)_\d+__)/)
    const result: React.ReactNode[] = []

    segments.forEach((segment, index) => {
      if (segment.startsWith("__BOLD_") || segment.startsWith("__STRONG_")) {
  ...
  const parseTable = (rows: string[]) => {
    try {
      const cleanRows = rows.filter((row) => row.trim() && !row.match(/^[\s\-|]+$/))
